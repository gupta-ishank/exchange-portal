...
public class MenuService {
                ...
                try {
                    ...
                    if (!Objects.isNull(val.getParameters())) {
                        ...
                        val.getParameters().forEach(parVal -> {
                            ...
                            eachValidation.put("maxLength", ((parVal.getSchema().getMaxLength() == null) ? null
                                    : parVal.getSchema().getMaxLength().toString()));
                            eachValidation.put("minLength", ((parVal.getSchema().getMinLength() == null) ? null
                    ...
                    if (!Objects.isNull(val.getRequestBody())) {
                        id = 100;
                        MediaType contentVal = val.getRequestBody().getContent().get("application/json");
                        if (contentVal != null) {
                ...
                if (!Objects.isNull(objMap)) {
                    objMap.forEach((key, value) -> {
                        if (mapper.convertValue(value, Map.class).get("type").equals("object")) {
                            map.put(key, filterRedundedData(mapper.convertValue(value, Map.class).get("properties"),
                                    "object"));
                            ...
                            map.put(key,
                                    filterRedundedData(mapper.convertValue(value, Map.class).get("items"), "array"));
                        } else {
            ...
            if (type.equals("object")) {
                if (!Objects.isNull(objMap)) {
                    objMap.forEach((key, value) -> {
                        ...
                        if (mapper.convertValue(value, Map.class).get("type").equals("object")) {
                            tableData.addAll(formatTableData2(mapper.convertValue(value, Map.class).get("properties"),
                                    id, requiredFileds, "object"));
                        } else if (mapper.convertValue(value, Map.class).get("type").equals("array")) {
                            tableData.addAll(formatTableData2(mapper.convertValue(value, Map.class).get("items"), id,
    ...
    public Object testing(String path) {
        ...
        // ;
        return formatTableData2(nullFieldFilter(store.getPaths().get("/mplace/selleritems").getPost().getSecurity()),
                id, new ArrayList<>(), "array");
    }
